...
public class DistributionGraphDensity extends ApplicationFrame {

	private static final long serialVersionUID = 9097047963618614545L;

	public DistributionGraphDensity(final String title, final CalcDistribution calc) {
		super(title);
	...
	private static XYSeries calcDistExp(CalcDistribution calc) {
		...
		int size = (int)(Math.log10(max)+1);
		int dx = (int)Math.pow(10, size - 1);
		int dy = (int)Math.pow(10, size - 2);
		
		for (int i = 0; i < Math.pow(10, size); i+=1000) {
			seriesExp.add(i + dx, exp.density(i + dy));
		}
		return seriesExp;
	}
	...
	private static XYSeries calcDistLog(CalcDistribution calc) {
		...
		for (int i = 0; i < Math.pow(10, size); i+=1000) {
			seriesLog.add(i + dx, log.density(i + dy));
		}
		return seriesLog;
	}
	...
	private static XYSeries calcDistGamma(CalcDistribution calc) {
		...
		for (int i = 0; i < Math.pow(10, size); i+=1000) {
			seriesGamma.add(i + dx, gammaDist.density(i + dy));
		}
		return seriesGamma;
	}
	...
	private static XYSeries calcDistWeibull(CalcDistribution calc) {
		...
		for (int i = 0; i < Math.pow(10, size); i+=1000) {
			seriesWeibull.add(i + dx, weibullDist.density(i + dy));
		}
		return seriesWeibull;
	}

	private static JFreeChart createChartHistogram(IntervalXYDataset intervalxydataset) {
		JFreeChart jfreechart = ChartFactory.createHistogram("Probabilty Density Function", "TBF(s)", "Frequency", intervalxydataset,
...
/*
	...
	public static void main(String args[]) throws IOException {
		...
		DistributionGraphDensity histogramdemo1 = new DistributionGraphDensity("Graph", calc);
		histogramdemo1.pack();
